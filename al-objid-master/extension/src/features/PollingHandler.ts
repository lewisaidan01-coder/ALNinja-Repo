...
import { CheckResponse } from "../lib/backend/CheckResponse";
...
export class PollingHandler implements Disposable {
    ...
    private async check() {
        ...
        const newsUpdates = this.processNews(updates);
        const appUpdates = await this.processAppUpdates(updates);
        const anyUpdates = newsUpdates || appUpdates;
    ...
    private processNews(updates: CheckResponse): boolean {
        const { _news } = updates;
        if (!_news) {
            return false;
        }
        return NewsHandler.instance.updateNews(_news);
    }

    private async processAppUpdates(updates: CheckResponse): Promise<boolean> {
        ...
        for (let appId of Object.keys(apps)) {
            ...
            if (await NotificationsFromLog.instance.updateLog(appId, _log, this._appName[appId])) {
                anyUpdates = true;
            }
        }
        return anyUpdates;
